...
class MemoryManager {
  ...
  public:
    ...
    void make_progress();
    bool is_idle(GPUStreamManager& streams) const;
    ...
    bool poll_request(Request& req, GPUEventSet& deps_out);
    void release_request(std::shared_ptr<Request> req, GPUEvent event = {});
  ...
  private:
    ...
    void unlock_access(MemoryId memory_id, Buffer& buffer, Request& req, GPUEvent event);

    void make_entry_valid(MemoryId memory_id, Buffer& buffer, GPUEventSet& deps_out);
    bool try_lock_access(MemoryId memory_id, Buffer& buffer, Request& req, GPUEventSet& deps_out);

    GPUEvent copy_h2d(DeviceId device_id, Buffer& buffer);
    GPUEvent copy_d2h(DeviceId device_id, Buffer& buffer);
