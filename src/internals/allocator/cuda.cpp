...
bool PinnedMemoryAllocator::allocate_impl(size_t nbytes, void*& addr_out) {
    ...
    CUresult result = cuMemHostAlloc(&addr_out, nbytes, CU_MEMHOSTALLOC_PORTABLE | CU_MEMHOSTALLOC_DEVICEMAP);
...
DevicePoolAllocator::DevicePoolAllocator(CudaContextHandle context, std::shared_ptr<CudaStreamManager> streams, size_t max_bytes):
    m_context(context),
    m_streams(streams),
